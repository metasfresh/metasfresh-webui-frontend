...
class Table extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (!disconnectFromState && !_.isEqual(prevState.selected, selected)) {
                        ...
                        rows &&
                            ...
                            rows.map(row => {
                                ...
                                if (row.indent.length > expandedDepth) {
                                    this.setState(prev => ({
                                        collapsedRows: prev.collapsedRows.concat(
                                            row[keyProperty]
                                        )
                                    }));
                                }
        ...
        if (this.table) {
            ...
            if (rowSelected.length > 0) {
                ...
                if (typeof idFocusedDown == 'number') {
                    rowSelected[rowSelected.length - 1].children[
                        idFocusedDown
                    ].focus();
                }
            }
        ...
        if (collapsed) {
            this.setState(prev => ({
                collapsedParentsRows: update(prev.collapsedParentsRows, {
                    $splice: [
                        [
                            prev.collapsedParentsRows.indexOf(
                                node[keyProperty]
                            ),
                            1
                        ]
            ...
            rows.map(item => {
                if (item.id === selected[0]) {
                    if (item.includedDocuments) {
                        ...
                        node = item;
                        isCollapsed =
                            collapsedParentsRows.indexOf(keyProp) > -1;
                    }
                }
            });
    ...
    render() {
        ...
        const { contextMenu, selected, promptOpen, isBatchEntry } = this.state;
        ...
        return (
            <div className="table-flex-wrapper">
                ...
                <DocumentListContextShortcuts
                    ...
                    handleDelete={
                        selected.length > 0 ? () => this.handleDelete() : ''
                    }
